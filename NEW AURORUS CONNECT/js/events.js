class EventManager {
    ...
    renderEvents() {
        ...
        tbody.innerHTML = sortedEvents.map(event => {
            ...
            return `
                <tr>
                    <td>${event.eventName || 'Unnamed Event'}</td>
                    <td>${eventDate ? eventDate.toLocaleDateString() : 'No date set'}</td>
                    <td>${event.location || 'No location'}</td>
                    <td><span class="event-status ${status.toLowerCase().replace(' ', '-')}">${status}</span></td>
                    <td>
                        <button class="btn-small" onclick="eventManager.editEvent('${event.id}')">Edit</button>
                        <button class="btn-small btn-danger" onclick="eventManager.deleteEvent('${event.id}')">Delete</button>
                    </td>
                </tr>
            `;
        }).join('');
